...
try:
    from modules.core_improvements import (
        FeatureNameSanitizer, 
        MulticollinearityHandler
    )
    MULTICOLLINEARITY_HANDLER_AVAILABLE = True
except ImportError:
    MULTICOLLINEARITY_HANDLER_AVAILABLE = False
    print("[WARNING] Could not import core_improvements features. Some features may not work.")
    ...
    class FeatureNameSanitizer:
        ...
        def sanitize_dataframe(self, df): 
            return df
        def restore_feature_importance(self, imp): 
            return imp
        def get_original_name(self, name): 
            return name
    
    class MulticollinearityHandler:
        def __init__(self, vif_threshold=10.0, correlation_threshold=0.9):
            self.vif_threshold = vif_threshold
            self.correlation_threshold = correlation_threshold
        
        def analyze_multicollinearity(self, X):
            return {"severity": "unknown", "recommended_models": []}
        
        def get_adaptive_model_recommendations(self, severity):
            return []
        
        def remove_redundant_features(self, X, method="vif"):
            return X, []
        
        def create_pca_features(self, X):
            return X, None
...
class ModelManager:
    ...
    def __init__(self, config: Config):
        ...
        # Initialize multicollinearity handler with proper config integration
        if MULTICOLLINEARITY_HANDLER_AVAILABLE:
            ...
            logger.info("Multicollinearity handler initialized in ModelManager")
        else:
            self.multicollinearity_handler = MulticollinearityHandler()  # Uses fallback
            logger.warning("Using fallback multicollinearity handler in ModelManager")
    ...
    def get_adaptive_model_selection(self, X: pd.DataFrame,
                                    available_models: Dict[str, List[str]],
                                    task_type: str,
                                    auto_handle_multicollinearity: bool = True,
                                    vif_threshold: float = 10.0,
                                    correlation_threshold: float = 0.9) -> Tuple[List[str], Dict[str, Any]]:
        """Enhanced model selection with VIF-based ARD prioritization"""
        
        mc_analysis = {"severity": "unknown", "recommended_models": []}
        
        if auto_handle_multicollinearity and self.multicollinearity_handler:
            logger.info("Performing multicollinearity analysis for adaptive model selection...")
            
            # Update thresholds
            self.multicollinearity_handler.vif_threshold = vif_threshold
            self.multicollinearity_handler.correlation_threshold = correlation_threshold
            
            # Analyze multicollinearity
            mc_analysis = self.multicollinearity_handler.analyze_multicollinearity(X)
            max_vif = max(mc_analysis.get("vif_scores", {}).values()) if mc_analysis.get("vif_scores") else 0
            
            logger.info(f"Multicollinearity detected: severity = {mc_analysis.get('severity', 'unknown')}, max VIF = {max_vif:.2f}")
            
            # ENHANCED: VIF-based model prioritization
            all_available = [model for models in available_models.values() for model in models]
            
            if max_vif > 20:  # High multicollinearity - prioritize ARD and regularized models
                logger.info(f"High multicollinearity (VIF: {max_vif:.2f}) - prioritizing ARD and regularized models")
                
                if task_type == 'regression':
                    priority_models = [
                        'ARD Regression',           # Top priority for high multicollinearity
                        'Bayesian Ridge',           # Second priority  
                        'Ridge',                    # Classical regularization
                        'Lasso',                    # Feature selection regularization
                        'ElasticNet',               # Combined regularization
                        'Random Forest',            # Tree-based (handles multicollinearity)
                    ]
                else:
                    priority_models = [
                        'Ridge Classifier',         # Regularized classification
                ...
                recommended_models = [m for m in priority_models if m in all_available]
                
                # Fill up to 6-8 models if needed
                remaining = [m for m in all_available if m not in recommended_models]
                recommended_models.extend(remaining[:8-len(recommended_models)])
                
                mc_analysis["recommended_models"] = recommended_models
                logger.info(f"High multicollinearity recommendations: {recommended_models}")
                
            else:
                # Use standard severity-based recommendations for moderate/low multicollinearity
                recommended_models = self.multicollinearity_handler.get_adaptive_model_recommendations(
                    mc_analysis['severity']
                )
                mc_analysis["recommended_models"] = recommended_models
            
            # Filter by availability
            all_available = [model for models in available_models.values() for model in models]
            adaptive_selection = [model for model in mc_analysis.get("recommended_models", []) if model in all_available]
            
            # Add some high-performing models if list is too short
            if len(adaptive_selection) < 4:
                backup_models = ["Random Forest", "Ridge", "XGBoost", "ElasticNet"]
                for model in backup_models:
                    if model in all_available and model not in adaptive_selection:
                        adaptive_selection.append(model)
                        if len(adaptive_selection) >= 6:
                            break
            
            mc_analysis["recommended_models"] = adaptive_selection
            
            logger.info(f"Multicollinearity severity: {mc_analysis['severity']}")
            logger.info(f"Recommended models: {adaptive_selection}")
            
            return adaptive_selection, mc_analysis
            
        else:
            # No multicollinearity handling - return default recommendation
            all_available = [model for models in available_models.values() for model in models]
            default_selection = all_available[:8]  # First 8 available models
            
            return default_selection, mc_analysis

    def apply_multicollinearity_treatment(self, X: pd.DataFrame, 
                                        mc_analysis: Dict[str, Any],
                                        treatment_method: str = "auto") -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """Apply multicollinearity treatment to features"""
        
        if not self.multicollinearity_handler or mc_analysis.get("severity") == "low":
            return X, {"treatment_applied": "none", "reason": "low or no multicollinearity"}
        
        treatment_info = {"treatment_applied": treatment_method, "original_features": len(X.columns)}
        
        try:
            if treatment_method == "auto":
                # Choose treatment based on severity
                if mc_analysis["severity"] == "high":
                    treatment_method = "remove_vif"
                elif mc_analysis["severity"] == "moderate":
                    treatment_method = "none"  # Let regularized models handle it
                else:
                    treatment_method = "none"
            
            if treatment_method == "remove_vif":
                X_treated, removed_features = self.multicollinearity_handler.remove_redundant_features(
                    X, method="vif"
                )
                treatment_info.update({
                    "method": "VIF-based removal",
                    "removed_features": removed_features,
                    "features_removed_count": len(removed_features),
                    "final_features": len(X_treated.columns)
                })
                logger.info(f"Removed {len(removed_features)} high-VIF features")
                return X_treated, treatment_info
                
            elif treatment_method == "remove_correlation":
                X_treated, removed_features = self.multicollinearity_handler.remove_redundant_features(
                    X, method="correlation"
                )
                treatment_info.update({
            ...
            elif treatment_method == "pca":
                X_treated, pca_transformer = self.multicollinearity_handler.create_pca_features(X)
                treatment_info.update({
                    "method": "PCA transformation",
                    "pca_transformer": pca_transformer,
                    "final_features": len(X_treated.columns),
                    "explained_variance": pca_transformer.explained_variance_ratio_.sum() if pca_transformer else 0
                })
                logger.info(f"Applied PCA: {len(X.columns)} -> {len(X_treated.columns)} components")
                return X_treated, treatment_info
                
            else:
                # No treatment
                treatment_info["treatment_applied"] = "none"
                return X, treatment_info
                
        except Exception as e:
            logger.error(f"Multicollinearity treatment failed: {e}")
            treatment_info.update({
                "treatment_applied": "failed",
                "error": str(e)
            })
            return X, treatment_info

    def get_multicollinearity_aware_recommendations(self, training_results: Dict[str, Any]) -> List[str]:
        """Generate recommendations that consider multicollinearity"""
        recommendations = []
        
        mc_analysis = training_results.get('multicollinearity_analysis', {})
        severity = mc_analysis.get('severity', 'unknown')
        
        if severity == 'high':
            recommendations.extend([
                "High multicollinearity detected in your data:",
                "• Linear models (Ridge, Lasso) performed well due to regularization",
                "• Tree-based models may show unstable feature importance",
                "• Consider feature engineering to create composite variables",
                "• SHAP values should be interpreted cautiously"
            ])
            
            # Check if regularized models performed well
            if training_results.get('models'):
                regularized_models = ['Ridge', 'Lasso', 'ElasticNet', 'Bayesian Ridge']
                top_3_models = sorted(
                    training_results['models'].items(),
                    key=lambda x: x[1]['test_scores'].get(training_results['primary_metric'], 0),
                    reverse=True
                )[:3]
                
                top_model_names = [name for name, _ in top_3_models]
                regularized_in_top = [m for m in top_model_names if m in regularized_models]
                
                if regularized_in_top:
                    recommendations.append(f"• Regularized models ({', '.join(regularized_in_top)}) handled multicollinearity well")
        
        elif severity == 'moderate':
            recommendations.extend([
                "Moderate multicollinearity detected:",
                "• Both regularized and tree-based models should work well",
                "• Monitor model stability across different data splits", 
                "• Feature importance is generally reliable but verify consistency"
            ])
        
        elif severity == 'low':
            recommendations.append("Low multicollinearity - all model types should perform reliably")
    ...
    def train_models(self, selected_models: List[str], processed_data: Dict,
        ...
        # Transfer multicollinearity analysis from processed_data
        if 'multicollinearity_analysis' in processed_data:
            results['multicollinearity_analysis'] = processed_data['multicollinearity_analysis']
            logger.info("Multicollinearity analysis transferred to training results")
